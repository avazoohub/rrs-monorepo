        ...
        <div className={`fixed top-0 right-0 z-[10] bg-[#201d27] w-11/12 md:w-10/12 lg:w-[30rem] h-full overflow-hidden transition ${selectedTeam ? 'translate-x-0' : 'translate-x-[100%]'}`}>
          <div className="absolute inset-0 z-2 w-full h-full flex flex-col justify-start overflow-y-auto">
            {team && selectedTeam && (
              <>
                <div className="px-10">
                  <button onClick={() => setSelectedTeam(undefined)} className="absolute top-0 right-0 m-4 bg-white/5 w-11 h-11 rounded-full  hover:bg-[#282430] active:bg-[#1b1821] active:scale-[0.97] transition"> &#x2715; </button>
                  <img
                    src={team.logos}
                    alt=""
                  ...
                  <p className="text-2xl text-center font-medium mb-8">
                    {team.displayName}
                    <span className="block text-center text-base"> {team.record?.items[1].summary} ({team.record?.items[1].stats[3].value.toFixed(2)}% winning rate) </span>
                  </p>
                  ...
                  <div>
                    <div className="flex items-center space-x-2 text-sm font-light">
                      ...
                      <div className="flex items-center space-x-2">
                        {teams && teams[0].needs.map((need : any) => (
                          <span className={`text-sm font-light flex items-center justify-center w-[2.5rem] h-[2.5rem] rounded-full ${need.isMet ? 'bg-white/60': 'bg-white/10'}`}>
                          {getPosition(need.positionId) ?? "--"}
                        </span>
                      ))} 
                      </div>
                    </div>
                    <p className="flex items-center space-x-2 divide-x divide-gray-100/20 text-sm font-medium mt-2 mb-4">
                    ...
                    {picks.map((pick: any) => (
                        <div
                          key={pick.id}
                          className={`grid grid-cols-4 py-2 w-full text-xs font-light  ${answer?.round === pick.round && answer?.pick === pick.pick && answer?.questionNumber === 1 ? "opacity-50 pointer-events-none cursor-not-allowed" : ""}`}
                        >
