...
export default function Sidebar() {
    ...
    return (
        <>
            <TopBar />
            <aside className={`fixed md:static bottom-0 left-0 right-0 z-[99] md:z-[1] w-full md:w-2/12 h-[50%] md:h-full overflow-y-auto py-6 md:py-0 px-6 md:px-2 md:rounded-2xl bg-[#0c0813] md:bg-transparent transition ${isOpen ? 'translate-y-[0]' : 'translate-y-[100%] md:translate-y-[0]'}`}>
                <div className="flex items-center justify-start space-x-4 mb-4 px-2">
                    <Image
                        src="/avatar/male.svg"
                        width={45}
                        height={45}
                        alt="Rewards Ripple Logo"
                        className="block rounded-full bg-white object-contain w-14 h-14"
                    />
                    <div>
                        <p className="heading font-medium text-lg md:text-base">
