...
export default function Answers() {
    ...
    const getAnswerDetails = (allAnswers: any, round: number, pick: number, questionId: number, userAnswer: string, status: number, answerId: number): any => {
        ...
        <div className="bg-[#110e19] rounded-2xl p-6">
            ...
            <div className="flex flex-col space-y-2 mt-4 divide-y divide-gray-100/5">
                {Array.isArray(answers) && !error && !isLoading && answers.map((answer: any, index: number) => {
                    ...
                    const answerDetails = getAnswerDetails(allAnswers, answer.round, answer.pick, answer.questionId, answer.answer, answer.status, answer.id);
                    ...
                    return (
                        <div key={answer.id} className="py-2">
                            <div className="grid grid-cols-8 gap-x-2 place-center items-center text-[#8b8795]">
                                <span className="col-span-3 text-white">{questionDetails.question}</span>
                                <span className="text-right">{round[0]} ({round[1]})</span>
