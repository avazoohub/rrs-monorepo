...
export default function Panel({
  ...
  return (
    <>
      <div
        className={`fixed bottom-0 md:bottom-auto md:top-0 left-0 md:left-auto right-0 z-[10] bg-[#201d27] w-full md:w-10/12 lg:w-[30rem] h-[80%] md:h-full rounded-t-2xl md:rounded-none overflow-hidden transition ${selectedTeam ? "translate-y-0 md:translate-x-0 md:translate-y-[0]" : "translate-y-[100%] md:translate-x-[100%] md:translate-y-[0]"}`}
      >
            ...
            <>
              <div className="px-10">
                <button
                  ...
                  className="fixed top-0 right-[0.3rem] m-4 bg-white/5 w-11 h-11 rounded-full  hover:bg-[#282430] active:bg-[#1b1821] active:scale-[0.97] transition"
                >
                  {" "}
                  &#x2715;{" "}
                </button>
                ...
                <p className="text-2xl text-center font-medium mb-8">
                  {team.displayName}
                  <span className="block text-center text-base font-light mt-1">
                    {" "}
                    {team.record?.items[1].summary} (
                    {team.record?.items[1].stats[3].value.toFixed(2)}% winning
                    rate){" "}
                  </span>
                </p>
                ...
                <div>
                  <div className="flex items-center space-x-2 font-light">
                    <p className="font-medium">Needs &rarr;</p>
                    <div className="flex items-center space-x-1 md:space-x-2">
                      {teams &&
                        teams[0].needs.map((need: any, index: number) => (
                          <span
                            key={index}
                            className={`text-sm font-light flex items-center justify-center w-[2.5rem] h-[2.5rem] rounded-full border border-[#18161d] ${need.isMet ? "bg-white/60" : "bg-[#2f2a3c]"}`}
                          >
                  ...
                  <p className="flex items-center space-x-2 divide-x divide-gray-100/20 font-medium mt-2 mb-3">
                    <span>
                      ...
                      <span className="text-green-600">
                        {" "}
                        ({team.record?.items[1].stats[3].value.toFixed(2)}){" "}
                      </span>{" "}
                    </span>
                    <span className="pl-2">
                      {" "}
                      Road: {team.record?.items[2].summary}{" "}
                      <span className="text-green-600">
                        {" "}
                        ({team.record?.items[2].stats[3].value.toFixed(2)}){" "}
                      </span>
                    </span>
                  </p>
                  <p className="font-medium mb-2">Overall Picks</p>
              ...
              <div className="grid grid-cols-5 md:grid-cols-6 mt-2 px-8">
                <p> No. </p>
      ...
      <div
        className={`flex fixed inset-0 bg-black/60 items-start justify-end transition ${selectedTeam ? "z-[9] opacity-100" : "z-[-1] opacity-0"}`}
      ></div>

      <QuestionModal
        refetch={refetch}
        answersRefetch={answersRefetch}
        openQuetions={openQuetions}
        setOpenQuestions={setOpenQuestions}
      />
    </>
  );
