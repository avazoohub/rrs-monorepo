...
export default function Questions({ index, question, answers, setOpenQuestions }: any) {
    ...
    return (
        <button
            ...
            className={`grid grid-cols-5 md:grid-cols-5 place-center w-full px-8 py-3 hover:bg-[#282430] active:bg-[#1b1821] active:scale-[0.97] transition ${hasAnswers(answer?.round, answer?.pick, question.id) ? 'text-[#919191] opacity-50 pointer-events-none cursor-not-allowed' : ''}`}
        >
